...
namespace server.Controllers
{
    ...
    public class TaskDependenciesController : ControllerBase
    {
        ...
        /// <returns>200 con la lista de <see cref="Projects.DependencyResponse"/>.</returns>
        [HttpGet]
        public IActionResult GetByTask([FromQuery] Guid? taskId)
        {
            ...
            var result = _repo.GetAll<Projects.DependencyResponse>("pm.SP_GetTaskDependencies", p);
            return Ok(result);
        }
        ...
        /// <returns>200 con la lista de <see cref="Projects.ProjectDependencyResponse"/>.</returns>
        [HttpGet("by-project")]
        public IActionResult GetByProject([FromQuery] Guid? projectId)
        {
            ...
            var result = _repo.GetAll<Projects.ProjectDependencyResponse>("pm.SP_GetProjectDependencies", p);
            return Ok(result);
        }
        ...
        /// <returns>200 con <see cref="Projects.SP_DependencyResult"/> o 400 si hay error (duplicado, ciclo, distinto proyecto).</returns>
        [HttpPost]
        [RequirePermission("projects.tasks.dependency-create", "Agregar dependencias entre tareas")]
        public IActionResult Insert([FromBody] Projects.DependencyRequest request)
        {
            var p = new DynamicParameters();
            p.Add("@TaskId",          request.TaskId);
            p.Add("@DependsOnTaskId", request.DependsOnTaskId);

            var result = _repo.Get<Projects.SP_DependencyResult>("pm.SP_InsertTaskDependency", p);
        ...
        /// <returns>200 con <see cref="Projects.SP_DependencyResult"/> o 400 si no existe.</returns>
        [HttpDelete("{id:guid}")]
        ...
        public IActionResult Delete(Guid id)
        {
            ...
            var result = _repo.Get<Projects.SP_DependencyResult>("pm.SP_DeleteTaskDependency", p);
