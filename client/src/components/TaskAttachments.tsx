...
/** Lista y gestiona los archivos adjuntos de una tarea existente (Azure Blob Storage). */
export function TaskAttachments({ taskId }: TaskAttachmentsProps) {
  const { user } = useAuth()
  const toast = useToast()
  const canUpload = usePermission('projects.tasks.attachment-upload')
  const canDelete = usePermission('projects.tasks.attachment-delete')
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await taskAttachmentsApi.list(taskId)
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true)
    try {
      const res = await taskAttachmentsApi.upload(taskId, file)
      if (!res.ok) {
  ...
  // Abre el archivo en una pestaña nueva con URL firmada; la pestaña se abre antes
  // del await para que el bloqueador de popups no la cancele.
  const handleOpen = async (att: Attachment) => {
    const win = window.open('', '_blank')
    try {
      const res = await taskAttachmentsApi.getUrl(att.id)
      if (!res.ok) {
        win?.close()
  ...
  const handleDelete = async (att: Attachment) => {
    const res = await taskAttachmentsApi.remove(att.id)
        ...
        <div className="space-y-1.5">
          {attachments.map(att => (
            <div key={att.id} className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-lg px-3 py-2">
              ...
              {att.uploadedByUserId === user?.userId && canDelete && (
